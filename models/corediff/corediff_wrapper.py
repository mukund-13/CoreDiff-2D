...
class adjust_net(nn.Module):
    def __init__(self, out_channels=64, middle_channels=32):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(1, middle_channels, 3, padding=1), #changed from 2 to 1
            nn.ReLU(inplace=True),
            nn.AvgPool2d(2),
...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1): # changed in from 2 to 1
...
class Network(nn.Module):
    ...
    def forward(self, x, t, y, x_end, adjust=True):
        ...
        # x_adjust = torch.cat((y, x_end), dim=1)  # Ensure y and x_end are [N, 1, H, W]
        # x_adjust = x_adjust.repeat(1, 3, 1, 1)  # Repeat the single channel three times to simulate RGB channels
        # print("X_adjust shape:", x_adjust.shape)
